...
func (fc *FabricChain) InterchainEventListener(chanErr chan *errors.ChanError) {
	...
	reg, eventch, err := fc.eventClient.RegisterBlockEvent(fi) //channelClient.RegisterChaincodeEvent(fc.ChainInfo.CrossChainCode, "[\\S\\s]*")  //
	if err != nil {
	...
	logging.Logger.Infof("RegisterBlockEvent SUCCESS")
...
func (fc *FabricChain) getServiceInfo(requestId string) (*serviceCallInfo, error) {
	...
	if fabres.TxValidationCode != pb.TxValidationCode_VALID {
		return nil, errors.New(fmt.Sprintf("call fabric ServiceInfo TxValidationCode is %s", fabres.TxValidationCode))
	}
	res := &serviceCallInfo{}
